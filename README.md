...
3. Repository-URL eintragen: **https://github.com/The88ers/ha-addon-hk-app** → **Hinzufügen**.
4. Im Store **HK Web App** auswählen → **Installieren** (der Supervisor baut das Image aus dem `Dockerfile` – kann einige Minuten dauern).
...
Ordner **`hk_web_app`** aus diesem Repo nach **`/addons/hk_web_app`** auf dem Home-Assistant-Host kopieren (Samba „addons“-Freigabe oder SSH). Im Store erscheint das Add-on unter **Lokale Add-ons**.
...
| `hk_web_app/config.yaml` | App: Ingress, `homeassistant_api`, Watchdog |
| `hk_web_app/Dockerfile` | Image (Alpine-Base, Node.js) |
| `hk_web_app/app/` | Node-Server, UI, Scheduler |
| `hk_web_app/rootfs/` | s6-Service (`run`) |
...
Details: **`hk_web_app/DOCS.md`**, Changelog: **`CHANGELOG.md`**.
